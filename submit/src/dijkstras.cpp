...
class Node {			//node class for implementing the algorithm
	public:
		int x;
		int y;
		int weight;
		int distance;	//sets these variables after initiating dijkstras
		char type;

		bool visited;
		Node *backedge;
		vector<Node *> adj;		//maps and vectors to store adjacent nodes and map edges
		multimap<int, Node *>::iterator spot_in_mm;
};

class Dijk {
	public:
		Dijk(int argc, char **argv, multimap<int, Node *> unvisited);		//initializing class
		vector<Node *> grid;
		map<char, int> terrain;
		int r;																//setting everythign from input
		int c;
		int start;
		int goal;
};
